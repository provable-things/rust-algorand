...
impl AlgorandTxGroup {
    ...
    fn to_hex(txns: Vec<AlgorandSignedTransaction>) -> Result<String> {
        Ok(txns
            .iter()
            .map(|t| t.to_hex())
...
mod tests {
    ...
    #[test]
    fn should_compute_the_correct_group_id_1() {
        let tx1 = AlgorandTransaction::asset_transfer(
            92741063,
            MicroAlgos(1000),
            1,
            ...
            AlgorandHash::testnet_genesis_hash().unwrap(),
            None,
            AlgorandAddress::from_str("EYFOOICWTFOSHAT6MHUR34INXKPYPMS3OREKXPH3ER5FPL7TIYT6QTPHZ4")
                .unwrap(),
        )
        .unwrap();
        assert_eq!(
            tx1.to_id().unwrap(),
            "V3JFC3IMHI5XWHUOTQSKZWDQSYK6HXBYO7ZB4HLLBW35EF2UMCTA"
        );
        let mut args: Vec<AlgorandApplicationArg> = Vec::new();
        args.push(AlgorandApplicationArg::from("issue"));
        args.push(
            AlgorandAddress::from_str("3XOLRWTASJY25KA6PVMAC3MQBWY4RW3HRAKTSL6ZXJJEJA4B2ODQP3OWGA")
                .unwrap()
                .to_application_arg(),
    ...
    #[test]
    fn should_compute_the_correct_group_id_2() {
        let tx1 = AlgorandTransaction::asset_transfer(
            92741063,
            MicroAlgos(1000),
            1,
        ...
        let mut args: Vec<AlgorandApplicationArg> = Vec::new();
        let arg2: u64 = 92769772;
        args.push(AlgorandApplicationArg::from("issue"));
        args.push(AlgorandApplicationArg::from(arg2));
        let mut accounts: Vec<AlgorandAddress> = Vec::new();
        accounts.push(
            AlgorandAddress::from_str("DW2JXC2PGOC7JVHSICF5U7BP2MNKM7ESAN52EPG3JIJRAACSAMGDLW6DII")
                .unwrap(),
        );
        let mut foreign_apps: Vec<u64> = Vec::new();
        foreign_apps.push(arg2);
        let mut foreign_assets: Vec<u64> = Vec::new();
        foreign_assets.push(92741063);
        let tx2 = AlgorandTransaction::application_call_noop(
            92734028,
